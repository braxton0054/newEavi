...
export default function WhatsAppSettingsPage() {
  ...
  return (
    <main className="px-6 py-6 max-w-3xl">
      ...
      <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-5"> Connect WhatsApp for automated messaging per campus.</p>
      ...
      {["MAIN", "WEST"].map(campus => {
        ...
        return (
          <div key={campus} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-xl p-5 mb-4">
            ...
            {!waLoaded || !wa ? (
              <p className="text-xs text-zinc-400 dark:text-zinc-500">Checking WhatsApp status...</p>
            ) : (
            <><div className="flex items-center gap-3 mb-1">
              <span className={`inline-block w-3 h-3 rounded-full ${wa.connected ? "bg-green-50 dark:bg-green-9500" : "bg-zinc-300"}`} />
              <span className="text-sm font-medium">{wa.connected ? "Connected" : "Not Connected"}</span>
            </div>
            {wa.connected && wa.phoneNumber && <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mb-3">WhatsApp: {wa.phoneNumber}</p>}
            {wa.qr && <div className="flex justify-center py-4"><img src={wa.qr} alt="QR" className="w-48 h-48" /></div>}
            {wa.connecting && !wa.qr && <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 text-center">Generating QR code...</p>}

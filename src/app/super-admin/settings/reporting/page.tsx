  ...
  return (
    <main className="px-4 sm:px-6 py-6 max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-base font-semibold text-zinc-900">Reporting Dates</h1>
          <p className="text-xs text-zinc-400 mt-0.5">Set reporting start and end dates per month. Shared across all campuses.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="rounded-lg border border-zinc-300 px-3 py-1.5 text-sm bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700 outline-none"
          >
      ...
      {loading ? (
        ...
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-zinc-50 border-b border-zinc-200">
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider w-32">Month</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Report Start</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Report End</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100">
                {periods.map(p => (
                  <tr key={p.month} className="hover:bg-zinc-50/50 transition-colors">
                    <td className="px-4 py-3 font-medium text-zinc-800 text-sm">{MONTHS[p.month - 1]}</td>
                    <td className="px-4 py-3">
                      <input
                        type="date"
                        value={p.startDate}
                        onChange={e => setDate(p.month, "startDate", e.target.value)}
                        className="w-full max-w-48 rounded-lg border border-zinc-200 px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700 outline-none"
                      />
                    </td>
                    <td className="px-4 py-3">
                      <input
          ...
          <div className="sm:hidden space-y-2">
            {periods.map(p => (
              <div key={p.month} className="bg-white border border-zinc-200 rounded-xl p-4">
                <p className="text-sm font-semibold text-zinc-800 mb-3">{MONTHS[p.month - 1]}</p>
                <div className="space-y-2">
                  <div>
                    <label className="block text-[11px] font-medium text-zinc-500 mb-1">Report Start</label>
                    <input type="date" value={p.startDate} onChange={e => setDate(p.month, "startDate", e.target.value)}
                      className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700 outline-none" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-medium text-zinc-500 mb-1">Report End</label>
          ...
          <div className="mt-6 flex items-center justify-between">
            <p className="text-xs text-zinc-400">{periods.filter(p => p.startDate || p.endDate).length} / 12 months configured</p>

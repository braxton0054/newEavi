...
export default function EmailSettingsPage() {
  ...
  if (loading || sysLoading) return <div className="flex items-center justify-center py-12 text-sm text-zinc-400">Loading...</div>;
  ...
  return (
    <main className="px-6 py-6 max-w-3xl">
      ...
      <p className="text-xs text-zinc-400 mb-5">Configure Gmail SMTP for system notifications and per-campus sending.</p>

      {/* System Email */}
      <div className="bg-white border border-zinc-200 rounded-xl p-5 mb-4">
        <h2 className="text-sm font-medium text-zinc-900 mb-1">System Email</h2>
        <p className="text-xs text-zinc-400 mb-4">Used for OTP, password resets, and automated notifications.</p>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">Gmail Address</label>
            <input type="email" value={sysEmail.user} onChange={e => setSysEmail(p => ({ ...p, user: e.target.value }))} placeholder="system@eavicollege.ac.ke" className="w-full rounded-lg border border-zinc-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700" />
          </div>
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">App Password{sysEmail.hasAppPassword && <span className="ml-2 text-xs text-green-600">(configured)</span>}</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="text" value={sysEmail.host} onChange={e => setSysEmail(p => ({ ...p, host: e.target.value }))} className="w-full rounded-lg border border-zinc-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700" />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Port</label>
              <input type="number" value={sysEmail.port} onChange={e => setSysEmail(p => ({ ...p, port: parseInt(e.target.value) || 587 }))} className="w-full rounded-lg border border-zinc-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700" />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">From Name</label>
              <input type="text" value={sysEmail.fromName} onChange={e => setSysEmail(p => ({ ...p, fromName: e.target.value }))} className="w-full rounded-lg border border-zinc-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700" />
            </div>
          </div>
        </div>

...
export default function BursarySettingsPage() {
  ...
  return (
    <main className="px-6 py-6 max-w-3xl">
      ...
      <p className="text-xs text-zinc-400 mb-5">Upload a bursary application PDF. The same form is shared across all campuses.</p>

      <div className="bg-white border border-zinc-200 rounded-xl p-5">
        {hasPdf ? (
          <div className="flex items-center gap-3 mb-4">
            <span className="inline-block w-3 h-3 rounded-full bg-green-500" />
            <span className="text-sm font-medium text-green-700">Bursary form uploaded</span>
            <button onClick={() => handleDeleteBursaryForm("MAIN")} className="text-sm text-red-600 hover:underline ml-auto">Remove</button>
          </div>
        ) : (
          <p className="text-sm text-zinc-400 mb-4">No bursary form PDF uploaded.</p>
        )}

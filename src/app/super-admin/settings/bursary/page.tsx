...
export default function BursarySettingsPage() {
  ...
  return (
    <main className="px-6 py-6 max-w-3xl">
      ...
      <p className="text-xs text-zinc-400 mb-5">Upload a bursary application PDF form for each campus.</p>

      {(["MAIN", "WEST"] as const).map(campus => {
        const f = form[campus] || { bursaryFormPdf: null };
        const hasPdf = f.bursaryFormPdf !== null;
        const campusLabel = campus === "MAIN" ? "Main Campus" : "West Campus";

        return (
          <div key={campus} className="bg-white border border-zinc-200 rounded-xl p-5 mb-4">
            <h2 className="text-sm font-medium text-zinc-900 mb-4">{campusLabel}</h2>
            {hasPdf ? (

    ...
    <>
      <header className="bg-white border-b border-zinc-200 px-6 lg:px-8 py-3.5 sticky top-0 z-30">
        <div className="flex items-center justify-between">
          <div className="lg:pl-0 pl-10">
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Upload reusable fee structure PDFs (stored in database)</p>
          </div>
      ...
      <main className="px-6 lg:px-8 py-6 max-w-4xl">
        <form onSubmit={handleUpload} className="bg-white rounded-xl border border-zinc-200 p-5 mb-5">
          <h2 className="text-sm font-medium text-zinc-900 mb-1">Upload fee structures</h2>
          <p className="text-xs text-zinc-400 mb-4">Select up to 15 PDF files — they will all be uploaded at once</p>
          <div className="mb-4">
            <label className="block text-[11px] font-medium text-zinc-500 mb-1">PDF files *</label>
            <input
              type="file"
              accept=".pdf"
              multiple
              onChange={handleFileChange}
              className="w-full text-xs text-zinc-500 file:mr-2 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
            />
          </div>

          {files.length > 0 && (
            <div className="mb-4 p-3 bg-blue-50/40 rounded-lg border border-blue-100">
              <p className="text-[11px] font-medium text-blue-700 mb-2">{files.length} file{files.length !== 1 ? "s" : ""} selected</p>
              <div className="space-y-1">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center justify-between text-xs text-zinc-600">
                    <span className="truncate">{f.name}</span>
                    <button type="button" onClick={() => removeFile(i)} className="text-red-500 hover:text-red-700 ml-2 shrink-0">&times;</button>
                  </div>
                ))}
              </div>
            </div>
                    ...
                    {renamingId === s.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 rounded-lg border border-zinc-200 px-2.5 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-700"
                          autoFocus
                          onKeyDown={e => {
                        ...
                        <button onClick={() => setRenamingId(null)} className="text-[11px] text-zinc-500 hover:underline">Cancel</button>
                      </div>
                    ) : (
